...
const child = spawn(executable, [
  ...
  '--mute-audio', '--use-fake-ui-for-media-stream', '--use-fake-device-for-media-stream', '--remote-debugging-port=0', `--user-data-dir=${profile}`, 'about:blank'
...
async function main() {
  ...
  const checks = {};
  await navigate('/'); checks.home = await evaluate(`({ cards: document.querySelectorAll('.tool-card').length, downloads: document.querySelectorAll('.tool-card__download').length, shares: document.querySelectorAll('.tool-card__share').length, subjects: document.querySelectorAll('#subject-list button').length, github: document.querySelector('[data-github-link]')?.href, count: document.querySelector('#result-count')?.textContent, newTab: document.querySelector('#tool-grid .tool-card__title a')?.target === '_blank', overflow: document.documentElement.scrollWidth > innerWidth })`);
  checks.share = await evaluate(`(() => { const button=document.querySelector('.share-button'); if(!button) return {hasButton:false}; button.click(); const dialog=document.querySelector('.share-dialog'); const result={hasButton:true,dialog:!!dialog,qr:!!document.querySelector('.share-qr__canvas'),links:document.querySelectorAll('.share-link').length,url:document.querySelector('.share-dialog__url input')?.value}; document.querySelector('.share-dialog .icon-btn')?.click(); result.closed=!document.querySelector('.share-dialog'); document.querySelector('.tool-card__share').click(); result.cardUrl=document.querySelector('.share-dialog__url input')?.value; document.querySelector('.share-dialog .icon-btn')?.click(); return result; })()`);
  checks.fuzzySearch = await evaluate(`(() => { const search=document.querySelector('#tool-search'); search.value='接晓'; search.dispatchEvent(new Event('input',{bubbles:true})); const result={cards:document.querySelectorAll('#tool-grid .tool-card').length,name:document.querySelector('#tool-grid .tool-card__title')?.textContent}; document.querySelector('#clear-search').click(); return result; })()`);
  ...
  await navigate('/tools/handwriting-digits/index.html'); checks.handwritingGated = await evaluate(`({disabled:document.querySelector('#startBtn').disabled,hint:document.querySelector('#startHint').textContent})`); await evaluate(`(() => { HTMLAnchorElement.prototype.click=function(){}; window.showDirectoryPicker=async()=>{const w={write:async()=>{},close:async()=>{}}; const h={name:'test',getDirectoryHandle:async()=>h,getFileHandle:async()=>({createWritable:async()=>w})}; return h;}; const id=document.querySelector('#studentId'); id.value='T001'; id.dispatchEvent(new Event('input',{bubbles:true})); document.querySelector('#selectFolderBtn').click(); return true; })()`); await delay(150); await evaluate(`(() => { document.querySelector('#startBtn').click(); document.querySelector('#saveBtn').click(); return true; })()`); await delay(700); checks.handwriting = await evaluate(`({count:document.querySelector('#collectedCount').textContent,max:document.querySelector('#maxCountLabel').textContent,status:document.querySelector('#status').textContent,exportVisible:!document.querySelector('#exportAllBtn').hidden,fullscreen:!!document.querySelector('#fullscreenBtn'),target:document.querySelector('#targetNumber').textContent,overflow:document.documentElement.scrollWidth>innerWidth})`);
  await navigate('/tools/noise-meter/index.html'); checks.noise = await evaluate(`(() => { const hasStart=!!document.querySelector('#startBtn'); document.querySelector('#startBtn').click(); return {hasStart,threshold:document.querySelector('#threshold').value,overflow:document.documentElement.scrollWidth>innerWidth}; })()`); await delay(700); checks.noiseStatus = await evaluate(`({status:document.querySelector('#status').textContent,stopEnabled:!document.querySelector('#stopBtn').disabled})`);
  await navigate('/tools/fullscreen-clock/index.html'); checks.clock = await evaluate(`({time:document.querySelector('#clockTime').textContent,date:document.querySelector('#clockDate').textContent,hasFullscreen:!!document.querySelector('#fullscreenBtn'),overflow:document.documentElement.scrollWidth>innerWidth})`);
  await navigate('/tools/pomodoro-timer/index.html'); checks.pomodoro = await evaluate(`(() => { const before=document.querySelector('#timerDisplay').textContent; document.querySelector('#startBtn').click(); document.querySelector('#resetBtn').click(); return {before,phase:document.querySelector('#phaseLabel').textContent,overflow:document.documentElement.scrollWidth>innerWidth}; })()`);
  await navigate('/tools/markdown-to-word/index.html'); checks.markdown = await evaluate(`(() => { HTMLAnchorElement.prototype.click=function(){}; const preview=document.querySelector('#preview'); const result={hasHeading:!!preview.querySelector('h1'),hasTable:!!preview.querySelector('table'),hasMath:!!preview.querySelector('math')}; document.querySelector('#convertBtn').click(); result.status=document.querySelector('#status').textContent; result.overflow=document.documentElement.scrollWidth>innerWidth; return result; })()`);
  await navigate('/tools/image-editor/index.html'); await evaluate(`(async () => { const canvas=document.createElement('canvas'); canvas.width=600; canvas.height=450; const ctx=canvas.getContext('2d'); const grad=ctx.createLinearGradient(0,0,600,450); grad.addColorStop(0,'#1e3a8a'); grad.addColorStop(1,'#f59e0b'); ctx.fillStyle=grad; ctx.fillRect(0,0,600,450); for(let i=0;i<6000;i+=1){ ctx.fillStyle='rgba('+(i%255)+','+((i*7)%255)+','+((i*13)%255)+',0.6)'; ctx.fillRect(Math.random()*600,Math.random()*450,3,3); } const blob=await new Promise((resolve)=>canvas.toBlob(resolve,'image/jpeg',0.9)); const file=new File([blob],'photo.jpg',{type:'image/jpeg'}); const transfer=new DataTransfer(); transfer.items.add(file); const input=document.querySelector('#fileInput'); input.files=transfer.files; input.dispatchEvent(new Event('change',{bubbles:true})); return true; })()`); await delay(900); checks.image = await evaluate(`({canvasVisible:!document.querySelector('#canvasWrap').hidden,meta:document.querySelector('#imageMeta').textContent,output:document.querySelector('#outputInfo').textContent,format:document.querySelector('#formatSelect').value,hasDownload:!document.querySelector('#downloadBtn').disabled,overflow:document.documentElement.scrollWidth>innerWidth})`); checks.imageQuality = await evaluate(`(async () => { const setQuality=async (q)=>{ const el=document.querySelector('#quality'); el.value=String(q); el.dispatchEvent(new Event('input',{bubbles:true})); await new Promise((r)=>setTimeout(r,700)); return document.querySelector('#outputInfo').textContent; }; const high=await setQuality(95); const low=await setQuality(40); return {high,low}; })()`); checks.imageCompress = await evaluate(`(async () => { document.querySelector('#targetSize').value='25'; document.querySelector('#compressBtn').click(); await new Promise((r)=>setTimeout(r,1800)); return {status:document.querySelector('#status').textContent,output:document.querySelector('#outputInfo').textContent}; })()`); checks.imagePngCompress = await evaluate(`(async () => { const format=document.querySelector('#formatSelect'); format.value='image/png'; format.dispatchEvent(new Event('change',{bubbles:true})); document.querySelector('#targetSize').value='30'; document.querySelector('#compressBtn').click(); await new Promise((r)=>setTimeout(r,2400)); return {status:document.querySelector('#status').textContent,output:document.querySelector('#outputInfo').textContent}; })()`);
  ...
  const unexpectedRequests = requests.filter((url) => /^https?:/i.test(url) && !url.startsWith(baseUrl) && !/\.workers\.dev\//i.test(url));
  const passed = checks.home.cards === 11 && checks.home.downloads === 11 && checks.home.shares === 11 && checks.home.subjects >= 2 && /github\.com\/xingyezn\/OpenEduTools/.test(checks.home.github) && checks.home.count === '找到 11 个工具' && checks.home.newTab && checks.share.hasButton && checks.share.dialog && checks.share.qr && checks.share.links >= 4 && checks.share.closed && /\/tools\//.test(checks.share.cardUrl || '') && checks.fuzzySearch.cards === 1 && checks.fuzzySearch.name === '随机点名' && checks.subjectFilter.cards === 1 && checks.subjectFilter.name === '成绩统计' && /已抽中/.test(checks.picker.status) && checks.group.groups === 3 && checks.timer.finished && checks.cleaner.output === '示例甲\n' && /已完成 5 个分数/.test(checks.score.status) && checks.handwritingGated.disabled === false && /学号/.test(checks.handwritingGated.hint) && checks.handwriting.count === '1' && checks.handwriting.max === '100' && /已保存/.test(checks.handwriting.status) && checks.handwriting.exportVisible && checks.handwriting.fullscreen && /^[0-9]$/.test(checks.handwriting.target) && checks.noise.hasStart && checks.noise.threshold === '60' && /测量中/.test(checks.noiseStatus.status) && checks.noiseStatus.stopEnabled && /^\d{2}:\d{2}:\d{2}$/.test(checks.clock.time) && checks.clock.date.length > 0 && checks.clock.hasFullscreen && checks.pomodoro.before === '25:00' && checks.pomodoro.phase === '专注' && checks.markdown.hasHeading && checks.markdown.hasTable && checks.markdown.hasMath && /已生成 Word/.test(checks.markdown.status) && checks.image.canvasVisible && /600/.test(checks.image.meta) && checks.image.hasDownload && checks.image.format === 'image/jpeg' && /预估约/.test(checks.image.output) && checks.imageQuality.high !== checks.imageQuality.low && /已压缩/.test(checks.imageCompress.status) && /已压缩/.test(checks.imagePngCompress.status) && !Object.values(checks).some((check) => check?.overflow) && errors.length === 0 && unexpectedRequests.length === 0;
