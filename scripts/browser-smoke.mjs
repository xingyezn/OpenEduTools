...
async function main() {
  ...
  await navigate('/tools/handwriting-digits/index.html'); await evaluate(`(() => { HTMLAnchorElement.prototype.click=function(){}; const id=document.querySelector('#studentId'); id.value='T001'; id.dispatchEvent(new Event('change',{bubbles:true})); document.querySelector('#saveBtn').click(); return true; })()`); await delay(700); checks.handwriting = await evaluate(`({count:document.querySelector('#collectedCount').textContent,status:document.querySelector('#status').textContent,exportVisible:!document.querySelector('#exportAllBtn').hidden,target:document.querySelector('#targetNumber').textContent,overflow:document.documentElement.scrollWidth>innerWidth})`);
