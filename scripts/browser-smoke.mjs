...
async function main() {
  ...
  await navigate('/tools/image-editor/index.html'); await evaluate(`(async () => { const canvas=document.createElement('canvas'); canvas.width=400; canvas.height=300; const ctx=canvas.getContext('2d'); ctx.fillStyle='#3366ff'; ctx.fillRect(0,0,400,300); ctx.fillStyle='#ffcc00'; ctx.fillRect(50,50,100,100); const blob=await new Promise((resolve)=>canvas.toBlob(resolve,'image/png')); const file=new File([blob],'test.png',{type:'image/png'}); const transfer=new DataTransfer(); transfer.items.add(file); const input=document.querySelector('#fileInput'); input.files=transfer.files; input.dispatchEvent(new Event('change',{bubbles:true})); return true; })()`); await delay(900); checks.image = await evaluate(`({canvasVisible:!document.querySelector('#canvasWrap').hidden,meta:document.querySelector('#imageMeta').textContent,output:document.querySelector('#outputInfo').textContent,hasDownload:!document.querySelector('#downloadBtn').disabled,overflow:document.documentElement.scrollWidth>innerWidth})`);
