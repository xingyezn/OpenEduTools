...
async function main() {
  ...
  const passed = checks.home.cards === 5 && checks.home.downloads === 5 && checks.home.subjects >= 2 && /github\.com\/xingyezn\/OpenEduTools/.test(checks.home.github) && checks.home.count === '找到 5 个工具' && checks.fuzzySearch.cards === 1 && checks.fuzzySearch.name === '随机点名' && checks.subjectFilter.cards === 1 && checks.subjectFilter.name === '成绩统计' && /已抽中/.test(checks.picker.status) && checks.group.groups === 3 && checks.timer.finished && checks.cleaner.output === '示例甲\n' && /已完成 5 个分数/.test(checks.score.status) && !Object.values(checks).some((check) => check?.overflow) && errors.length === 0 && unexpectedRequests.length === 0;
