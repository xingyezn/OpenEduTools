...
async function main() {
  ...
  const checks = {};
  await navigate('/'); checks.home = await evaluate(`({ cards: document.querySelectorAll('.tool-card').length, count: document.querySelector('#result-count')?.textContent, overflow: document.documentElement.scrollWidth > innerWidth })`);
  ...
  const passed = checks.home.cards === 5 && checks.home.count === '找到 5 个工具' && /已抽中/.test(checks.picker.status) && checks.group.groups === 3 && checks.timer.finished && checks.cleaner.output === '示例甲\n' && /已完成 5 个分数/.test(checks.score.status) && !Object.values(checks).some((check) => check?.overflow) && errors.length === 0 && unexpectedRequests.length === 0;
