...
async function main() {
  ...
  checks.share = await evaluate(`(() => { const button=document.querySelector('.share-button'); if(!button) return {hasButton:false}; button.click(); const dialog=document.querySelector('.share-dialog'); const result={hasButton:true,dialog:!!dialog,qr:!!document.querySelector('.share-qr__canvas'),links:document.querySelectorAll('.share-link').length,url:document.querySelector('.share-dialog__url input')?.value}; document.querySelector('.share-dialog .icon-btn')?.click(); result.closed=!document.querySelector('.share-dialog'); return result; })()`);
