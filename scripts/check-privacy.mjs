...
function walk(directory) { return fs.readdirSync(directory, { withFileTypes: true }).flatMap((entry) => { const full = path.join(directory, entry.name); if (['.git','node_modules','tests','scripts'].includes(entry.name)) return []; return entry.isDirectory() ? walk(full) : [full]; }); }
...
for (const file of runtimeFiles) {
  ...
  for (const match of text.matchAll(/fetch\s*\(\s*["']([^"']+)/g)) if (match[1] !== './data/tools.json') failures.push(`${relative}: 包含未批准的网络读取 ${match[1]}`);
