(function (root) {
  'use strict';
  const state = { tools: [], ranked: [], query: '', category: 'all', subject: 'all', onlyFavorites: false, stats: null, page: 1 };
  const PAGE_SIZE = 9;
  ...
  function initCatalog(data, stats) {
    ...
    const ids = state.tools.map((tool) => tool.id); let storage; try { storage = localStorage; } catch { storage = undefined; } const favorites = root.OETFavorites.createFavorites(storage); const recent = root.OETRecent.createRecent(storage);
    ...
    function scrollToCatalog() {
      const target = byId('catalog-title'); if (!target || typeof target.scrollIntoView !== 'function') return;
