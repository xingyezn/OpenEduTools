(function (root) {
  'use strict';
  const CONFIG = { CANVAS_SIZE: 400, OUTPUT_SIZE: 50, MAX_COLLECTION: 100, THRESHOLD: 128, DATA_FOLDER: 'data', STORAGE_KEY: 'openEduTools:tool:handwriting-digits:settings' };
  ...
  const elements = Object.fromEntries(['studentId', 'drawingCanvas', 'previewCanvas', 'targetNumber', 'collectedCount', 'currentIndex', 'binaryData', 'status', 'clearBtn', 'saveBtn', 'exportAllBtn', 'clearAllBtn', 'selectFolderBtn', 'folderInfo', 'folderPath', 'remember'].map((id) => [id, document.getElementById(id)]));
  const drawCtx = elements.drawingCanvas.getContext('2d');
  const previewCtx = elements.previewCanvas.getContext('2d');
  const state = { studentId: '', currentIndex: 1, collectedData: [], isDrawing: false, currentTarget: 0, directoryHandle: null, useFileSystemAPI: false };
  let resetTimer = 0;
  ...
  function updateUI() {
    ...
    const done = state.currentIndex > CONFIG.MAX_COLLECTION;
    elements.saveBtn.disabled = done; elements.saveBtn.textContent = done ? '已完成 100 张' : '保存 / 下一个';
  ...
  function persistSettings() {
    ...
    try { localStorage.setItem(CONFIG.STORAGE_KEY, JSON.stringify({ studentId: state.studentId, currentIndex: state.currentIndex })); } catch { /* 存储不可用时忽略 */ }
  }
  ...
  async function saveCurrent() {
    ...
    if (state.currentIndex > CONFIG.MAX_COLLECTION) { setStatus('已完成 100 张收集。', 'success'); return; }
  ...
  function init() {
    ...
    elements.studentId.addEventListener('change', (event) => { state.studentId = event.target.value.trim(); persistSettings(); });
    document.addEventListener('keydown', (event) => {
      const tag = (event.target.tagName || '').toUpperCase();
      if (['INPUT', 'TEXTAREA', 'SELECT'].includes(tag)) return;
      if (event.key === 'Delete' || event.key === 'Backspace') { event.preventDefault(); clearCanvas(true); }
      else if (event.key === 'Enter' || event.code === 'Space') { event.preventDefault(); saveCurrent(); }
    });
  }
  init();
