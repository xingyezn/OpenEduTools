(function (root) {
  ...
  function start() { if (state.running) return; try { if (state.finished) reset(); state.mode = elements.mode.value; if (state.mode === 'countdown' && state.accumulated === 0) { state.duration = selectedDuration(); if (state.duration === 0) throw new Error('倒计时不能为 0，请设置时长。'); } state.startedAt = Date.now(); state.running = true; elements.finished.hidden = true; elements.start.disabled = true; elements.pause.disabled = false; elements.pause.textContent = '暂停'; setStatus('计时中。', 'success'); root.OpenEduAnalytics?.toolUse?.('classroom-timer'); ticker = setInterval(render, 200); render(); } catch (error) { elements.start.disabled = false; setStatus(error.message, 'error'); } }
